...
describe('A quantidade de produtos adicionados ao carrinho deve ser limitada pela quantidade disponível em estoque', () => {
  it('should increase quantity in shopping cart list until the availability quantity', async () => {
    ...
    expect(screen.getAllByTestId('shopping-cart-product-quantity')[0]).toHaveTextContent('0');
    fireEvent.click(screen.getAllByTestId('product-increase-quantity')[0]);
    fireEvent.click(screen.getAllByTestId('product-increase-quantity')[0]);
    fireEvent.click(screen.getAllByTestId('product-increase-quantity')[0]);
    expect(screen.getAllByTestId('shopping-cart-product-quantity')[0]).toHaveTextContent(mockedQueryResult.results[0].available_quantity);
  });
});
